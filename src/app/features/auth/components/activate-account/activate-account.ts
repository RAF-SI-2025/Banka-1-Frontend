 ...
 * Used when a new employee receives an activation link via email.
 */
@Component({
...
export class ActivateAccount implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams
      ...
      .subscribe(params => {
        ...
        if (!this.token) {
          this.tokenError.set('Invalid or missing activation link. Please contact your administrator.');
        }
      });
  }
  ...
  /**
   * Handles form submission for account activation.
   */
  onSubmit(): void {
    ...
    if (!this.token) {
      this.errorMessage.set('Invalid activation link. Please contact your administrator.');
      return;
    }
  ...
  hasPasswordMismatch(): boolean {
    return this.activateForm.hasError('passwordMismatch') &&
           this.activateForm.get('confirmPassword')?.touched === true;
  }
  ...
  get hasMinLength(): boolean {
    const value = this.activateForm.get('newPassword')?.value || '';
